...
if np.any(uygun_sartlar):
    ...
    tum_gercek_indexler = np.where(uygun_sartlar)[0]
    kopmalar = np.where(np.diff(tum_gercek_indexler) > 1)[0]
    if len(kopmalar) > 0:
        start=uygun_zaman[0].strftime('%H:%M')
        end=uygun_zaman[kopmalar[0]].strftime('%H:%M')
    else:
        start=uygun_zaman[0].strftime('%H:%M')
...
if puan >= 80:
    print(f"🔭 Puan: {puan:.2f}, için gökyüzü ve hava  koşulları MÜKEMMEL.")
elif puan >= 50:
    print(f"👀 Puan: {puan:.2f} , Gözlem yapılabilir, gökyüzü ve hava koşulları ORTALAMA.")
elif puan >= 30:
    print(f"⚠️ Puan: {puan:.2f} , Gözlem zorlayıcı olabilir, gökyüzü ve hava koşulları KÖTÜ.")
else:
    print(f"❌ Puan: {puan:.2f} , Gözlem güncel gökyüzü ve hava koşulları ile yapılamaz.")